...
process extract_variant_info {
    ...
    if (params.vcf_has_R2_field) {
        """
        set +o pipefail; bcftools +fill-tags --threads ${task.cpus} $vcf | bcftools query -f '%CHROM\\t%POS\\t%ID\\t%REF\\t%ALT\\t%TYPE\\t%AC\\t%AN\\t%MAF\\t%R2\\n' | uniq | gzip > ${qtl_subset}.variant_information.txt.gz
        """
    } else {
        """
        set +o pipefail; bcftools +fill-tags --threads ${task.cpus} $vcf | bcftools query -f '%CHROM\\t%POS\\t%ID\\t%REF\\t%ALT\\t%TYPE\\t%AC\\t%AN\\t%MAF\\tNA\\n' | uniq | gzip > ${qtl_subset}.variant_information.txt.gz
        """
    }
}